...
- Product creation currently relies on the in-memory mock API and doesn't prevent duplicate SKUs, so adding the same product code twice silently overwrites expectations until the process is restarted.
